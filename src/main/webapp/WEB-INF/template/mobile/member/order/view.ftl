					...
					[#else]
						[#if order.status == "PENDING_PAYMENT"]
							[#if order.groupBuying?? && order.amountPaid <= 0 && order.groupBuying.hasEnded()]
								<span class="pull-right text-orange">${message("GroupBuying.groupEnded")}</span>
							...
							[#elseif order.groupBuying?? && order.amountPaid > 0 && order.groupBuying.groupFailure]
								<span class="pull-right text-red">${message("GroupBuying.groupFailure")}</span>
							[#else]
								<span class="pull-right text-orange">${message("Order.Status." + order.status)}</span>
							[/#if]
						[#elseif order.status == "PENDING_REVIEW"]
							[#if order.groupBuying?? && order.groupBuying.status == "PENDING_GROUP_SUCCESS" && !order.groupBuying.hasEnded()]
								<span class="pull-right text-orange">${message("GroupBuying.Status.PENDING_GROUP_SUCCESS")}</span>
						...
						[#elseif order.status == "FAILED" || order.status == "DENIED"]
							<span class="pull-right text-red">${message("Order.Status." + order.status)}</span>
						[#elseif order.status == "CANCELED"]
							[#if order.groupBuying??]
								<span class="pull-right text-red">${message("GroupBuying.groupFailure")}</span>
							[#else]
								<span class="pull-right text-gray-dark">${message("Order.Status." + order.status)}</span>
							[/#if]
						[#else]
							<span class="pull-right text-green">${message("Order.Status." + order.status)}</span>
						[/#if]
					[/#if]
				</div>
			</div>
