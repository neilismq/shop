						...
						<span class="pull-right">
							<%if (order.status === "PENDING_PAYMENT") {%>
								<%if (order.groupBuying && order.amountPaid <= 0 && order.groupBuying.hasEnded) {%>
									<span class="text-orange">${message("GroupBuying.groupEnded")}</span>
								<%} else if (order.groupBuying && order.amountPaid > 0 && order.groupBuying.groupEnded) {%>
									<span class="text-orange">${message("GroupBuying.groupEnded")}</span>
								<%} else if (order.groupBuying && order.amountPaid > 0 && order.groupBuying.groupFailure) {%>
							...
							<%} else if (order.status === "PENDING_REVIEW") {%>
								<%if (order.groupBuying && order.groupBuying.status === "PENDING_GROUP_SUCCESS" && !order.groupBuying.hasEnded) {%>
									<span class="text-orange">${message("GroupBuying.Status.PENDING_GROUP_SUCCESS")}</span>
								<%} else if (order.groupBuying && order.groupBuying.groupFailure) {%>
									<span class="text-red">${message("GroupBuying.groupFailure")}</span>
								<%} else {%>
									<span class="text-orange"><%-statusText(order.status)%></span>
								<%}%>
