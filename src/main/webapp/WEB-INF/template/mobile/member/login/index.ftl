	...
	[#noautoesc]
		[#escape x as x?js_string]
			...
			$().ready(function() {
				...
				// 底部
				$footer.velocity("transition.slideUpBigIn", 400);
				...
				$loginPlugin.click(function() {
					...
					location.href = URI("${base}/social_user_login").setSearch({loginPluginId: loginPluginId, redirectUrl: redirectUrl}).toString();
					return false;
				});
	...
	<footer class="text-center">
		[#if loginPlugins?has_content && !socialUserId?has_content && !uniqueId?has_content]
			[#list loginPlugins as loginPlugin]
				<a href="javascript:;" data-login-plugin-id="${loginPlugin.id}" [#if loginPlugin.description??] title="${loginPlugin.description}"[/#if]>
