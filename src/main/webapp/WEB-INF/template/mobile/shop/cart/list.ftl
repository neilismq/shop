			...
			$().ready(function() {
				...
				[#if currentCart?? && !currentCart.isEmpty()]
					$.setCurrentWebviewStyle("titleNView.buttons[0]", {
						text: "${message("shop.cart.modify")}",
                        fontSize: "16px",
						onclick: "javascript:window.plus.webview.getWebviewById('cart-list').evalJS('$(document).trigger(\"clickModifyButton.shopxx.titleNView\")')"
					});
				[#else]
					$.setCurrentWebviewStyle("titleNView.buttons", []);
				[/#if]
				...
				function toggleHeaderButton() {
					...
					if ($modify.hasClass("active")) {
						if ($.isHtml5Plus()) {
							$.setCurrentWebviewStyle("titleNView.buttons[0].text", "${message("shop.cart.complete")}");
						} else {
							$modify.text("${message("shop.cart.complete")}");
						}
						...
						if ($.isHtml5Plus()) {
							$.setCurrentWebviewStyle("titleNView.buttons[0].text", "${message("shop.cart.modify")}");
						} else {
							$modify.text("${message("shop.cart.modify")}");
						}
				...
				// 切换顶部按钮
				$document.on("clickModifyButton.shopxx.titleNView", toggleHeaderButton);

				// 切换顶部按钮
				$modify.click(toggleHeaderButton);
