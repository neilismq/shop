...
<head>
	...
	<script id="receiverListTemplate" type="text/template">
		<%_.each(receivers, function(receiver, i) {%>
			<div class="<%-receiver.id == currentReceiverId ? "active " : ""%>list-group-item" data-uncalculate="true" data-receiver="<%-JSON.stringify(receiver)%>">
				<div class="media">
					<div class="media-body media-middle">
						<h5 class="media-heading">
							<%-receiver.consignee%>
		...
		[#escape x as x?js_string]
			...
			$().ready(function() {
				...
				$selection.on("click", ".list-group-item", function() {
					var $element = $(this);
					var uncalculated = $element.data("uncalculated");
	...
	<div id="mainPage" class="main-page page">
		<form id="orderForm" class="ajax-form" action="${base}/order/create" method="post">
			[#if skuId??]
				...
				<input name="groupBuyingId" type="hidden" value="${groupBuyingId!''}">
