				...
				<strong>
					<%if (product.type === "GENERAL") {%>
						<%if (product.groupBuying) {%>
							${message("Product.groupBuyingPrice")}
							<%-$.currency(product.groupBuyingPrice, true)%>
							<del><%-$.currency(product.defaultSku.price, true)%></del>
						<%} else {%>
		...
		[#escape x as x?js_string]
			...
			$().ready(function() {
				...
				var data = {
					...
					promotionPluginId: "${promotionPluginId}",
					orderType: null
				};
				...
				$filterButton.click(function() {
					...
					$filter.velocity("transition.slideRightBigIn", 400).parent().velocity("fadeIn");
				});
				
				// 筛选
				$filterComplete.click(function() {
					$filter.velocity("transition.slideRightBigOut", 400).parent().velocity("fadeOut");
				});
...
<body class="shop product-list">
	...
	<header class="header-default" data-spy="scrollToFixed">
		<div class="container-fluid">
			<div class="row html5plus-hidden">
				<div class="col-xs-1">
