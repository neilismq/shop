													...
													<li>
														[#if order.status == "PENDING_PAYMENT"]
															[#if order.groupBuying?? && order.amountPaid <= 0 && order.groupBuying.hasEnded()]
																<strong class="text-orange">${message("GroupBuying.groupEnded")}</strong>
															...
															[#elseif order.groupBuying?? && order.amountPaid > 0 && order.groupBuying.groupFailure]
																<strong class="text-red">${message("GroupBuying.groupFailure")}</strong>
															[#else]
																<strong class="text-orange">${message("Order.Status." + order.status)}</strong>
															[/#if]
														[#elseif order.status == "PENDING_REVIEW"]
															[#if order.groupBuying?? && order.groupBuying.status == "PENDING_GROUP_SUCCESS" && !order.groupBuying.hasEnded()]
																<strong class="text-orange">${message("GroupBuying.Status.PENDING_GROUP_SUCCESS")}</strong>
														...
														[#elseif order.status == "FAILED" || order.status == "DENIED"]
															<strong class="text-red">${message("Order.Status." + order.status)}</strong>
														[#elseif order.status == "CANCELED"]
															[#if order.groupBuying??]
																<strong class="text-red">${message("GroupBuying.groupFailure")}</strong>
															[#else]
																<strong class="text-gray-dark">${message("Order.Status." + order.status)}</strong>
															[/#if]
														[#else]
															<strong class="text-green">${message("Order.Status." + order.status)}</strong>
														[/#if]
														[#if order.hasExpired()]
