...
<head>
	...
	<title>${message("business.groupBuyingPromotionPlugin.add")} - Powered By SHOP++</title>
		...
		[#escape x as x?js_string]
			...
			$().ready(function() {
				...
				$productId.selectpicker({
					...
					ajax: {
						url: "${base}/business/group_buying_promotion/product_select",
						type: "GET",
						data: function() {
							return {
								keyword: "{{{q}}}"
				...
				$groupBuyingPromotionForm.validate({
					...
					submitHandler: function(form) {
						bootbox.confirm("${message("business.groupBuyingPromotionPlugin.addGroupBuyingConfirm")}", function(result) {
							if (result == null || !result) {
								return;
							}
							$(form).ajaxSubmit({
								successRedirectUrl: "${base}/business/group_buying_promotion/list?promotionPluginId=${promotionPluginId}"
							});
						});
					}
				});
	...
	<main>
		<div class="container-fluid">
			<ol class="breadcrumb">
				...
				<li class="active">${message("business.groupBuyingPromotionPlugin.add")}</li>
			</ol>
			<form id="groupBuyingPromotionForm" class="ajax-form form-horizontal" action="${base}/business/group_buying_promotion/save" method="post">
				<input name="promotionPluginId" type="hidden" value="${promotionPluginId}">
				<div class="panel panel-default">
					<div class="panel-body">
						<ul class="nav nav-tabs">
							<li class="active">
								<a href="#base" data-toggle="tab">${message("business.groupBuyingPromotionPlugin.base")}</a>
							</li>
							<li>
						...
						<div class="tab-content">
							<div id="base" class="tab-pane active">
								...
								<div class="form-group">
									...
									<div class="col-xs-9 col-sm-4">
										<div class="input-group" data-provide="datetimerangepicker" data-date-format="YYYY-MM-DD HH:mm:ss">
											<input id="beginDate" name="beginDate" class="form-control" type="text" autocomplete=“false”>
											<span class="input-group-addon">-</span>
											<input id="endDate" name="endDate" class="form-control" type="text" autocomplete=“false”>
										</div>
									</div>
								</div>
								<div class="form-group">
									...
									<div class="col-xs-9 col-sm-4">
										<select id="discountType" name="discountType" class="selectpicker form-control" data-size="10">
											[#list discountTypes as discountType]
												<option value="${discountType}">${message("GroupBuyingPromotionPlugin.DiscountType." + discountType)}</option>
											[/#list]
										</select>
									</div>
								</div>
								<div class="form-group">
									...
									<div class="col-xs-9 col-sm-4">
										<input id="discounValue" name="discounValue" class="form-control" type="text" maxlength="16">
									</div>
								</div>
								<div class="form-group">
									<label class="col-xs-3 col-sm-2 control-label item-required" for="purchasingLimit">${message("GroupBuying.groupSize")}:</label>
									<div class="col-xs-9 col-sm-4">
										<input id="groupSize" name="groupSize" class="form-control" type="text" maxlength="9">
									</div>
								</div>
								<div class="form-group">
									<label class="col-xs-3 col-sm-2 control-label" for="purchasingLimit">${message("GroupBuying.purchasingLimit")}:</label>
									<div class="col-xs-9 col-sm-4">
										<input id="purchasingLimit" name="purchasingLimit" class="form-control" type="text" maxlength="9">
									</div>
								</div>
								<div class="form-group">
									<label class="col-xs-3 col-sm-2 control-label item-required">${message("Promotion.products")}:</label>
									<div class="col-xs-9 col-sm-4">
										<select id="productId" name="productId" class="form-control" title="${message("business.promotionPlugin.productTitle")}"></select>
									</div>
								</div>
								<div class="form-group">
