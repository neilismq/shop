...
<head>
	...
	<title>${message("business.moneyOffPromotionPlugin.add")} - Powered By SHOP++</title>
		...
		[#escape x as x?js_string]
			...
			$().ready(function() {
				...
				$productIds.selectpicker({
					...
					ajax: {
						url: "${base}/business/money_off_promotion/product_select",
						type: "GET",
						data: function() {
							return {
								keyword: "{{{q}}}"
	...
	<main>
		<div class="container-fluid">
			<ol class="breadcrumb">
				...
				<li class="active">${message("business.moneyOffPromotionPlugin.add")}</li>
			</ol>
			<form id="moneyOffPromotionForm" class="ajax-form form-horizontal" action="${base}/business/money_off_promotion/save" method="post">
						...
						<div class="tab-content">
							<div id="base" class="tab-pane active">
								...
								<div class="hidden-element form-group">
									...
									<div class="col-xs-9 col-sm-4">
										<input id="conditionValue" name="conditionValue" class="form-control" type="text" maxlength="16" disabled>
									</div>
								</div>
								<div class="form-group">
									...
									<div class="col-xs-9 col-sm-4">
										<select id="discountType" name="discountType" class="selectpicker form-control" data-size="10">
											[#list discountTypes as discountType]
												<option value="${discountType}">${message("MoneyOffPromotionPlugin.DiscountType." + discountType)}</option>
											[/#list]
										</select>
									</div>
								</div>
