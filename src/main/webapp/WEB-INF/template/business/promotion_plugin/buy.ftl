			...
			$().ready(function() {
				...
				[#if currentStore.endDate??]
					...
					setInterval(function() {
						$.ajax({
							url: URI("${base}/business/promotion_plugin/end_date").setSearch("promotionPluginId", "${promotionPlugin.id}").toString(),
							type: "GET",
							dataType: "json",
							success: function(data) {
								if (moment(data.endDate).isAfter(currentEndDate)) {
									location.href = URI("${base}/business/promotion_plugin/list").setSearch("promotionPluginId", "${promotionPlugin.id}").toString();
								}
							}
						});
					}, 10000);
						...
						[#if currentStore.endDate??]
							[#if pluginEndDate??]
								<div class="form-group">
									<label class="col-xs-3 col-sm-2 control-label">${message("business.store.promotionPluginEndDate")}:</label>
