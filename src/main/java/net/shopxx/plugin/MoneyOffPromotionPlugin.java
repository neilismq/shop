 ...
 * FileId: L/ZFMLloq2zuQ4Y3aoUKr6AAcORuVdj7
 */
		...
		if (promotion != null && price != null && price.compareTo(BigDecimal.ZERO) > 0 && quantity > 0) {
			...
			case FIX_PRICE:
				try {
					...
					result = new BigDecimal(String.valueOf(groovyShell.evaluate(String.format(FIX_PRICE_EXPRESSION, discounValue))), MathContext.DECIMAL32);
				} catch (Exception e) {
					return price;
				}
				break;
