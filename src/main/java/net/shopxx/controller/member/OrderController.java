 ...
 * FileId: MbnLu8RJQVuuFJug6Xo+pGX/ySQqBBoG
 */
...
public class OrderController extends BaseController {
	...
	public ResponseEntity<?> cancel(@ModelAttribute(binding = false) Order order, @CurrentUser Member currentUser) {
		...
		if ((!Order.Status.PENDING_PAYMENT.equals(order.getStatus()) || (order.getGroupBuying() != null && order.getGroupBuying().hasEnded() && !order.getGroupBuying().isGroupEnded() && BigDecimal.ZERO.compareTo(order.getAmountPaid()) > 0))
				&& (!Order.Status.PENDING_REVIEW.equals(order.getStatus()) || (order.getGroupBuying() != null && GroupBuying.Status.PENDING_GROUP_SUCCESS.equals(order.getGroupBuying().getStatus())))) {
			return Results.NOT_FOUND;
		}
