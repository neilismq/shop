 ...
 * FileId: 5cOAovq+4rykZrNNj8LsCyWA01Okq/iS
 */
...
public class PaymentController extends BaseController {
	 ...
	 * 获取支付插件
	 */
	@GetMapping("/getPaymentPlugin")
	public ResponseEntity<?> getPaymentPlugin() {
		...
		if (CollectionUtils.isNotEmpty(paymentPlugins)) {
			...
			for (PaymentPlugin paymentPlugin : paymentPlugins) {
				...
				item.put("description", paymentPlugin.getDescription());
				content.add(item);
			}
			data.put("content", content);
			data.put("defaultPaymentPluginId", defaultPaymentPlugin.getId());
		}
