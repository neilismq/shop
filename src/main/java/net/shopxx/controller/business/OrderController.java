 ...
 * FileId: yb2eTMNWxAT74+zoMZ9iWMRls+eSJe58
 */
...
public class OrderController extends BaseController {
	...
	public ResponseEntity<?> review(@ModelAttribute(binding = false) Order order, Boolean passed, @CurrentUser Business currentUser) {
		...
		if (order.getGroupBuying() != null && !GroupBuying.Status.GROUP_SUCCESSED.equals(order.getGroupBuying().getStatus())) {
			return Results.UNPROCESSABLE_ENTITY;
		}
	 ...
	 */
	@GetMapping("/list")
	public String list(Order.Type type, Order.Status status, String memberUsername, Long groupBuyingId, Boolean isPendingReceive, Boolean isPendingRefunds, Boolean isAllocatedStock, Boolean hasExpired, Pageable pageable, @CurrentStore Store currentStore, ModelMap model) {
		...
		if (groupBuying != null && !groupBuying.getPromotion().getStore().equals(currentStore)) {
			throw new UnauthorizedException();
		}
		...
		if (StringUtils.isNotEmpty(memberUsername) && member == null) {
			...
			model.addAttribute("page", orderService.findPage(type, status, currentStore, member, null, groupBuying, isPendingReceive, isPendingRefunds, null, null, isAllocatedStock, hasExpired, pageable));
		}
