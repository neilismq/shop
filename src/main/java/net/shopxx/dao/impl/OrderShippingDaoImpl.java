 ...
 * FileId: zeBGjGBHNU9Cbe3L+EIW2dnVvujwoVIl
 */
...
public class OrderShippingDaoImpl extends BaseDaoImpl<OrderShipping, Long> implements OrderShippingDao {
	...
	public OrderShipping findLast(Order order) {
		...
		String jpql = "select orderShipping from OrderShipping orderShipping where lower(orderShipping.order) = lower(:order) order by orderShipping.createdDate desc";
		try {
