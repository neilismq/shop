 ...
 * FileId: dtc0wQEj3mPqgqGADQz4YSO/oD3D/R9m
 */
...
public class OrderDaoImpl extends BaseDaoImpl<Order, Long> implements OrderDao {
	...
	@Override
	public List<Order> findList(Order.Type type, Boolean isPrepaid, Boolean isGroupFailure, Integer first, Integer count, List<Filter> filters, List<net.shopxx.Order> orders) {
		CriteriaBuilder criteriaBuilder = entityManager.getCriteriaBuilder();
		CriteriaQuery<Order> criteriaQuery = criteriaBuilder.createQuery(Order.class);
		Root<Order> root = criteriaQuery.from(Order.class);
		criteriaQuery.select(root);
		...
		if (isPrepaid != null) {
			if (isPrepaid) {
				restrictions = criteriaBuilder.and(restrictions, criteriaBuilder.greaterThan(root.<BigDecimal>get("amountPaid"), root.<BigDecimal>get("refundAmount")));
			} else {
				restrictions = criteriaBuilder.and(restrictions, criteriaBuilder.lessThanOrEqualTo(root.<BigDecimal>get("amountPaid"), root.<BigDecimal>get("refundAmount")));
			}
		}
		if (isGroupFailure != null) {
			Subquery<GroupBuying> subquery = criteriaQuery.subquery(GroupBuying.class);
			Root<GroupBuying> subqueryRoot = subquery.from(GroupBuying.class);
			subquery.select(subqueryRoot);
			if (isGroupFailure) {
				Date now = new Date();
				Predicate subqueryRestrictions = criteriaBuilder.conjunction();
				subqueryRestrictions = criteriaBuilder.and(subqueryRestrictions, criteriaBuilder.equal(subqueryRoot, root.get("groupBuying")));
				subqueryRestrictions = criteriaBuilder.and(subqueryRestrictions, criteriaBuilder.equal(subqueryRoot.get("status"), GroupBuying.Status.PENDING_GROUP_SUCCESS));
				subqueryRestrictions = criteriaBuilder.and(subqueryRestrictions, criteriaBuilder.lessThanOrEqualTo(subqueryRoot.<Date>get("endDate"), now));
				subquery.where(subqueryRestrictions);
			} else {
				Predicate subqueryRestrictions = criteriaBuilder.conjunction();
				subqueryRestrictions = criteriaBuilder.and(subqueryRestrictions, criteriaBuilder.equal(subqueryRoot, root.get("groupBuying")));
				subqueryRestrictions = criteriaBuilder.and(subqueryRestrictions, criteriaBuilder.equal(subqueryRoot.get("status"), GroupBuying.Status.GROUP_SUCCESSED));
				subquery.where(subqueryRestrictions);
			}
	...
	@Override
	public Page<Order> findPage(Order.Type type, Order.Status status, Store store, Member member, Product product, GroupBuying groupBuying, Boolean isPendingReceive, Boolean isPendingRefunds, Boolean isUseCouponCode, Boolean isExchangePoint, Boolean isAllocatedStock, Boolean hasExpired,
			Pageable pageable) {
		CriteriaBuilder criteriaBuilder = entityManager.getCriteriaBuilder();
		CriteriaQuery<Order> criteriaQuery = criteriaBuilder.createQuery(Order.class);
		Root<Order> root = criteriaQuery.from(Order.class);
		...
		if (groupBuying != null) {
			restrictions = criteriaBuilder.and(restrictions, criteriaBuilder.equal(root.get("groupBuying"), groupBuying));
		}
		if (isPendingReceive != null) {
			Predicate predicate = criteriaBuilder.and(criteriaBuilder.or(root.get("expire").isNull(), criteriaBuilder.greaterThan(root.<Date>get("expire"), new Date())), criteriaBuilder.equal(root.get("paymentMethodType"), PaymentMethod.Type.CASH_ON_DELIVERY),
					criteriaBuilder.notEqual(root.get("status"), Order.Status.COMPLETED), criteriaBuilder.notEqual(root.get("status"), Order.Status.FAILED), criteriaBuilder.notEqual(root.get("status"), Order.Status.CANCELED), criteriaBuilder.notEqual(root.get("status"), Order.Status.DENIED),
	...
	@Override
	public Long count(GroupBuying groupBuying, Member member, List<Order.Status> statuses) {
		CriteriaBuilder criteriaBuilder = entityManager.getCriteriaBuilder();
		CriteriaQuery<Order> criteriaQuery = criteriaBuilder.createQuery(Order.class);
		Root<Order> root = criteriaQuery.from(Order.class);
		criteriaQuery.select(root);
		...
		if (CollectionUtils.isNotEmpty(statuses)) {
			restrictions = criteriaBuilder.and(restrictions, root.get("status").in(statuses));
		}
		criteriaQuery.where(restrictions);
	...
	@Override
	public Integer grantedPurchasedQuantity(GroupBuying groupBuying, Member member, Product.Type productType, Boolean hasExpired, Order.Status... excludeStatus) {
		CriteriaBuilder criteriaBuilder = entityManager.getCriteriaBuilder();
		CriteriaQuery<Integer> criteriaQuery = criteriaBuilder.createQuery(Integer.class);
		Root<OrderItem> root = criteriaQuery.from(OrderItem.class);
		criteriaQuery.select(criteriaBuilder.sum(root.<Integer>get("quantity")));
		Predicate restrictions = criteriaBuilder.conjunction();
		...
		Predicate orderSubqueryRestrictions = criteriaBuilder.conjunction();
		if (groupBuying != null) {
			orderSubqueryRestrictions = criteriaBuilder.and(orderSubqueryRestrictions, criteriaBuilder.equal(orderSubqueryRoot.get("groupBuying"), groupBuying));
		}
		if (member != null) {
			orderSubqueryRestrictions = criteriaBuilder.and(orderSubqueryRestrictions, criteriaBuilder.equal(orderSubqueryRoot.get("member"), member));
		}
		if (hasExpired != null) {
			if (hasExpired) {
				orderSubqueryRestrictions = criteriaBuilder.and(orderSubqueryRestrictions, orderSubqueryRoot.get("expire").isNotNull(), criteriaBuilder.lessThanOrEqualTo(orderSubqueryRoot.<Date>get("expire"), new Date()));
			} else {
				orderSubqueryRestrictions = criteriaBuilder.and(orderSubqueryRestrictions, criteriaBuilder.or(orderSubqueryRoot.get("expire").isNull(), criteriaBuilder.greaterThan(orderSubqueryRoot.<Date>get("expire"), new Date())));
			}
		}
		if (ArrayUtils.isNotEmpty(excludeStatus)) {
			orderSubqueryRestrictions = criteriaBuilder.and(orderSubqueryRestrictions, criteriaBuilder.not(orderSubqueryRoot.get("status").in(Arrays.asList(excludeStatus))));
		}

		orderSubquery.where(orderSubqueryRestrictions);
		restrictions = criteriaBuilder.and(restrictions, criteriaBuilder.in(root.get("order")).value(orderSubquery), criteriaBuilder.equal(root.get("type"), productType));
		criteriaQuery.where(restrictions);
	...
	@Override
	public Long grantedGroupBuyingParticipants(Order.Type type, GroupBuying groupBuying, Member member, List<Order.Status> statuses) {
		CriteriaBuilder criteriaBuilder = entityManager.getCriteriaBuilder();
		CriteriaQuery<Order> criteriaQuery = criteriaBuilder.createQuery(Order.class);
		Root<Order> root = criteriaQuery.from(Order.class);
		criteriaQuery.select(root);
